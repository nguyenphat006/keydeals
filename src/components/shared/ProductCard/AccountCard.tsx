...
export default function AccountCard({ product }: AccountCardProps) {
  return (
    <div className="glass-card group rounded-xl overflow-hidden transition-all duration-200 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-accent/20">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          {product.discount && product.discount > 0 && (
            <div className="absolute top-2 left-2 rounded-full bg-accent px-2 py-0.5 text-xs font-bold text-accent-foreground">
              -{product.discount}%
            </div>
          )}
